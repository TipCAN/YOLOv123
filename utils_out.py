...
def decoder(pred, grid_num=7, B=2, device='cpu', thresh=0.3, nms_th=0.5, gt=False):
    ...
    pred = pred.squeeze(0)
    pred_new = pred.clone()
    pred_new[:,:,0] = pred[:,:,4].clone()
    pred_new[:,:,1] = pred[:,:,9].clone()
    pred_new[:,:,2:6] = pred[:,:,0:4].clone()
    pred_new[:,:,6:10] = pred[:,:,5:9].clone()
    pred_new[:,:,10:] = pred[:,:,10:].clone()
    # pred = pred.unsqueeze(0)
    # print(pred.shape)
    contain1 = pred_new[:,:,0].unsqueeze(2)
    contain2 = pred_new[:,:,1].unsqueeze(2)
    ...
    for i in range(grid_num):
        for j in range(grid_num):
            for b in range(B):
                if mask[i,j,b] == 1:
                    box = pred_new[i, j, B+b*4 : B+b*4+4]
                    contain_prob = torch.FloatTensor([pred_new[i,j,b]]).to(device)
                    ...
                    box_xy[2:] = box[:2] + 0.5*box[2:]
                    max_prob, cls_index = torch.max(pred_new[i,j,5*B:],0)
...
def run_test_mAP(YOLONet, target, test_datasets, data_len, S=7, device='cuda:0', reversed=False, logger=None, little_test=None):
    ...
    with torch.no_grad():
        for i, (images, now_target, fname) in tqdm(enumerate(test_datasets)):
            ...
            pred = YOLONet(images_cuda[None, :, :, :])
            # #
            # boxes, clss, confs = decoder(now_target, grid_num=S, device=device, thresh=0.05, nms_th=.2)
            # mean = torch.tensor([0.485, 0.456, 0.406], dtype=torch.float32)
            # std = torch.tensor([0.229, 0.224, 0.225], dtype=torch.float32)
            # un_normal_trans = transforms.Normalize((-mean / std).tolist(), (1.0 / std).tolist())
            # img = un_normal_trans(images.squeeze(0))
            # img = draw_debug_rect(img.squeeze(0).permute(1, 2, 0), boxes, clss, confs,re=True, if_show=False)
            # boxes, clss, confs = decoder(pred, grid_num=S, device=device, thresh=0.05, nms_th=.2)
            # draw_debug_rect(img, boxes, clss, confs, color=(255, 0, 0), if_show=True)

            # if reversed:
            #     pred = convert_input_tensor_dim(pred)
            bboxes, clss, confs = decoder(pred, grid_num=S, device=device, thresh=0.01, nms_th=.3)
...
def draw_debug_rect(img, bboxes, clss, confs, color=(0, 255, 0), show_time=10000, re=False, if_show=True):
