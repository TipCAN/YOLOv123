    ...
    while e < num_epochs:
        ...
        with tqdm.tqdm(range(len(train_loader))) as pbar:
            for i, data_batch in enumerate(train_loader):
                ...
                loss, loss_xy, loss_wh, loss_Cobj, loss_Cnoobj= criterion.forward(SSgrid_pre, SSgrid_gt)
                ...
                lossinfo = 'l%0.2f,l_xy%0.2f,l_wh%0.2f,l_Cobj%0.2f,l_Cnoobj%0.2f'
                pbar.set_description(lossinfo%(loss_total/(i+1),loss_xy_total/(i+1),loss_wh_total/(i+1),loss_Cobj_total/(i+1),loss_Cnoobj_total/(i+1)),False)
                pbar.update()
#         logger.info(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))
#         logger.info('epoch: %d,lr: %.5f , loss: %.5f' % (e, learning_rate, loss_total))
        if e % save_epoch == 0:
