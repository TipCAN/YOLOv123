...
def yolov2_decoder(pred, img_size, conf=0.02, nms_threshold=0.5):
    '''
    pred_cls = [C*2,S,S]
    pred_response = [2,S,S]
    pred_bboxes = [4*2,S,S]
    ...
    class_num, h, w = pred_cls.shape
    box_num = pred_response.shape[0]
    class_num = int(class_num/box_num)


    pred_cls = pred_cls.view(box_num, class_num, h,w).permute(2,3,0,1).contiguous().view(-1, class_num)
    pred_bboxes = pred_bboxes.view(box_num,4, h, w).permute(2,3,0,1).contiguous().view(-1, 4)
    pred_response = pred_response.view(box_num,1, h, w).permute(2,3,0,1).contiguous().view(-1, 1)
